...
import { PowCoinGameModal } from "@/components/PowCoinGameModal";
...
export default function ShopEstrellasPage() {
  ...
  const [powGameOpen, setPowGameOpen] = useState(false);
  const [powGameRunning, setPowGameRunning] = useState(false);
  const [powGameAlumnoId, setPowGameAlumnoId] = useState<string | null>(null);
  ...
  const changeCount = useCallback(
    async (index: number, key: keyof Alumno, delta: number) => {
      if (!canEdit) return;
      if (key === "id" || key === "nombre" || key === "created_at") return;
  ...
  const startPowGame = useCallback(
    async (alumnoIndex: number) => {
      if (!canEdit || powGameRunning || rollingRef.current || rouletteOpen) return;
      const a = alumnos[alumnoIndex];
      if (!a) return;
      if (a.pow < 1) {
        alert("Necesitas al menos 1 POW para jugar.");
        return;
      }

      const updated = { ...a, pow: Math.max(0, a.pow - 1) };
      setPowGameAlumnoId(updated.id);
      setPowGameRunning(true);

      const nextList = [...alumnos];
      nextList[alumnoIndex] = updated;
      setAlumnos(nextList);
      ...
      try {
        ...
        alert("No se pudo activar el juego POW.");
        return;
      }
  ...
  const finishPowGame = useCallback(
    async (coinsCollected: number) => {
      if (!powGameAlumnoId) return;
      const a = alumnos.find((x) => x.id === powGameAlumnoId);
      if (!a) return;

      const updated = { ...a, monedas: a.monedas + coinsCollected };
      try {
        await saveAlumno(updated);
      } catch {
    ...
    (e: React.PointerEvent, index: number, key: string) => {
      ...
      function step(now: number) {
        ...
        if (ringEl) {
          ...
          if (circle) {
            circle.style.background = `conic-gradient(var(--gold) ${
              progress * 360
            }deg, rgba(255,255,255,0.06) ${progress * 360}deg)`;
          }
        }
        if (progress >= 1) {
  ...
  if (!supabase) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-black p-8 font-sans text-white">
  ...
  return (
    <div className="min-h-screen bg-black font-sans text-white">
      <header className="flex flex-wrap items-center gap-3 px-4 pb-2 pt-2 md:px-6 md:pb-3 md:pt-3">
        ...
        />
        <h1 className="text-sm md:text-base lg:text-lg flex-1 min-w-0">Shop_estrellas(v6.2)</h1>
      </header>
      ...
      <main className="max-w-4xl mx-auto p-4 md:p-6">
                ...
                {ITEMS_FOR_DISPLAY.map((it) => (
                  <div
                    ...
                    className={`item-box relative w-24 bg-[#1e50c5] rounded-xl p-2 text-center overflow-visible transition-transform ${
                      canEdit ? "cursor-pointer hover:scale-105" : "cursor-default opacity-95"
                    }`}
                    data-key={it.key}
                    data-index={i}
                    onClick={() => {
                  ...
                  >
                    <div className="ring hidden absolute inset-1.5 rounded-lg flex items-center justify-center pointer-events-none">
                      <div
                        ...
                        style={{
                          background: "conic-gradient(var(--gold) 0deg, rgba(255,255,255,0.06) 0deg)",
                        }}
                      />
                    </div>
                    <img
                    ...
                    />
                    <div className="text-xs">{(a as unknown as Record<string, number>)[it.key] ?? 0}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {alumnos.length === 0 && (
          <p className="text-sm text-white/70 mt-6">
            {canEdit ? "No hay alumnos. Anade uno arriba." : "No hay alumnos registrados."}
          </p>
        )}
      </main>
      ...
      {rouletteOpen && (
        ...
        >
          ...
          <div className="flex flex-1 items-center justify-center px-3 py-6 md:px-6">
            <div
              className={`flex flex-wrap items-center justify-center gap-3 sm:gap-4 md:gap-5 lg:gap-6 max-w-5xl ${
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-center rounded-xl p-2 md:p-3 transition-all duration-150 ${
                      active ? "scale-110 opacity-100" : "scale-100 opacity-45 md:opacity-50"
                    }`}
                  >
                    <img
      ...
      {powGameOpen && powGameAlumnoId && (
        <PowCoinGameModal
          timeSeconds={15}
          playerImageSrc={`${IMAGE_PATH}/logo_efrendrums.png`}
          ...
          onComplete={async (coins) => {
            await finishPowGame(coins);
          }}
        />
      )}
    </div>
